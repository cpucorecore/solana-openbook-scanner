...
//		errCnt = 0
//		start = time.Now()
//		resp, err := cli.GetBlockHeight(ctx)
...
//
//		blockHeight := resp.Result
//		if blockHeight <= cursor {
//			log.Logger.Warn(fmt.Sprintf("sol GetBlockHeight remote height %d <= local height %d", resp, cursor))
//			time.Sleep(time.Second * 1)
//			continue
//		}
//		log.Logger.Info(fmt.Sprintf("sol GetBlockHeight: cursor %d, remote height %d", cursor, resp))
//
//		for height := cursor + 1; height <= blockHeight; height++ {
//			taskCh <- height
//		}
//
//		cursor = blockHeight
...
//			//end = time.Now()
//			if err != nil {
//				log.Logger.Error(fmt.Sprintf("get block with slot:%d err:%s", task, err.Error()))
//			}
//
//			if resp.Error != nil {
//				log.Logger.Error(fmt.Sprintf("get block with slot:%d err:%s", task, resp.Error.Error()))
//			}
//
//			blockData, err := json.Marshal(resp.Result)
//			if err != nil {
//				log.Logger.Error(fmt.Sprintf("json marshal err:%s", err.Error()))
//			}
//			log.Logger.Info(string(blockData))
//			f, err := os.Create(fmt.Sprintf("block%d.json"))
//			if err != nil {
//				log.Logger.Error(fmt.Sprintf("create file err:%s", err.Error()))
//			}
//			f.Write(blockData)
//			f.Close()
